...
class DataProductRoleAssignment(Base, BaseORM, EventTrackedMixin):
    ...
    __table_args__ = (
        UniqueConstraint("data_product_id", "identity_id", name="unique_data_product_assignment"),
    )

    id = Column(UUID, primary_key=True, default=uuid4)
    data_product_id: Mapped[UUID] = mapped_column("data_product_id", ForeignKey("data_products.id"))
    data_product: Mapped["DataProduct"] = relationship(foreign_keys=[data_product_id])
    identity_id: Mapped[UUID] = mapped_column("identity_id", ForeignKey("identities.id"))
    identity: Mapped["Identity"] = relationship(back_populates="data_product_roles", foreign_keys=[identity_id])
    role_id: Mapped[UUID] = mapped_column("role_id", ForeignKey("roles.id"))
    role: Mapped["Role"] = relationship(foreign_keys=[role_id])
    decision: Mapped[DecisionStatus] = mapped_column(Enum(DecisionStatus), default=DecisionStatus.PENDING)
