    ...
    def sync_role(self, create, extracted, **kwargs):
        if self.decision == DecisionStatus.APPROVED:
            ...
            if self.role_id == ADMIN_UUID:
                authorizer.assign_admin_role(user_id=str(self.identity_id)) # TODO migrate to identity_id
            else:
                authorizer.assign_global_role(
                    role_id=str(self.role_id), user_id=str(self.identity_id) # TODO migrate to identity_id
                )
